...
<h1><span>{if !isset($email_create)}{l s='Log in'}{else}{l s='Create an account'}{/if}</span></h1>
...
{/literal}
	...
	<div class="row-fluid">
		...
		<form action="{$link->getPageLink('authentication', true)}" method="post" id="create-account_form_old" class="std span6">
			<fieldset class="titled_box">
				...
				<div class="form_content clearfix">
					...
					<p class="submit">
						...
						<input type="submit" id="SubmitCreate" name="SubmitCreate" class="btn" value="{l s='Create an account'}" />
		...
		<form action="{$link->getPageLink('authentication', true)}" method="post" id="login_form" class="std span6">
			<fieldset class="titled_box">
				<legend>{l s='Already registered?'}</legend>
				<div class="form_content clearfix">
					<p class="text">
					...
					<p class="submit">
						{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
						<input type="submit" id="SubmitLogin" name="SubmitLogin" class="btn btn-primary" value="{l s='Log in'}" />
					</p>
					<p class="lost_password"><a class="" href="{$link->getPageLink('password')}">{l s='Forgot your password?'}</a></p>
				</div>
			</fieldset>
		</form>
	</div>
	...
	<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")}" method="post" id="new_account_form" class="std clearfix">
		<fieldset>
			<legend>{l s='Instant Checkout'}</legend>
					...
					<div class="row-fluid">
						...
						<!--
							...
							<p class="required text">
								...
								<input type="tel" class="text" name="phone" id="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{/if} "/>
							</p>
							{/if}
							{/foreach}
		...
		<fieldset class="account_creation dni titled_box">
			<legend>{l s='Tax identification'}</legend>
			<p class="required text">
	...
	<form action="{$link->getPageLink('authentication', true)}" method="post" id="account-creation_form" class="std">
			...
			<div class="row-fluid after-box">
				<div class="span6">
					<p class="required text">
						...
						<input onkeyup="$('#firstname').val(this.value);" type="text" class="text span8" id="customer_firstname" name="customer_firstname" value="{if isset($smarty.post.customer_firstname)}{$smarty.post.customer_firstname}{/if}" />
					</p>
					<p class="required text">
						<label for="customer_lastname">{l s='Last name'} <sup>*</sup></label>
						<input onkeyup="$('#lastname').val(this.value);" type="text" class="text span8" id="customer_lastname" name="customer_lastname" value="{if isset($smarty.post.customer_lastname)}{$smarty.post.customer_lastname}{/if}" />
					</p>
				</div>
				<div class="span6">
					<p class="required text">
						<label for="email">{l s='Email'} <sup>*</sup></label>
						<input type="email" class="text span8" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}" />
					</p>
					<p class="required password clearfix">
						<label for="passwd">{l s='Password'} <sup>*</sup><span class="form_info">{l s='(5 characters min.)'}</span></label>
						<input type="password" class="text span8" name="passwd" id="passwd" />
			...
			<div class="row-fluid">
				<div class="span6">
					<p class="select">
						...
						<select class="span2" id="days" name="days">
							<option value="">-</option>
							{foreach from=$days item=day}
							<option value="{$day}" {if ($sl_day == $day)} selected="selected"{/if}>{$day}&nbsp;&nbsp;</option>
							{/foreach}
						...
						{l s='December'}
						*}
						<select class="span4" id="months" name="months">
							<option value="">-</option>
							{foreach from=$months key=k item=month}
							<option value="{$k}" {if ($sl_month == $k)} selected="selected"{/if}>{l s=$month}&nbsp;</option>
							{/foreach}
						</select>
						<select class="span3" id="years" name="years">
							<option value="">-</option>
							{foreach from=$years item=year}
							<option value="{$year}" {if ($sl_year == $year)} selected="selected"{/if}>{$year}&nbsp;&nbsp;</option>
							{/foreach}
				...
				<div class="span6">
					<p class="radio required">
						<p>{l s='Title'}</p>
						{foreach from=$genders key=k item=gender}
						<label for="id_gender{$gender->id}" class="top radio">
							<input type="radio" name="id_gender" id="id_gender{$gender->id}" value="{$gender->id}" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id}checked="checked"{/if} />
							{$gender->name}
						</label>
						{/foreach}
		...
		{if $b2b_enable}
		<fieldset class="account_creation titled_box">
			<legend>{l s='Your company information'}</legend>
			<p class="text">
		...
		<fieldset class="account_creation">
			...
			<p class="required text">
				...
				<p class="{if $one_phone_at_least}required {/if} text">
					...
					<input type="tel" class="text" name="phone_mobile" id="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{/if}" />
				</p>
			...
			<fieldset class="account_creation dni">
				...
				<p class="required text">
					...
					<input type="tel" class="text" name="dni" id="dni" value="{if isset($smarty.post.dni)}{$smarty.post.dni}{/if}" />
					<span class="form_info">{l s='DNI / NIF / NIE'}</span>
				</p>
			</fieldset>
			{/if}
			...
			<p class="cart_navigation required submit">
				...
				{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
				<input type="submit" name="submitAccount" id="submitAccount" value="{l s='Register'}" class="exclusive btn btn-primary" />
				<span><sup>*</sup>{l s='Required field'}</span>
			</p>
		</form>
		{/if}