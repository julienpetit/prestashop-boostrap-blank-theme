...
{if $block == 1}
	...
	{foreach from=$cms_titles key=cms_key item=cms_title}
		<div id="informations_block_left_{$cms_key}" class="well-white informations_block_left">
			<h4 class="block-title"><a href="{$cms_title.category_link}">{if !empty($cms_title.name)}{$cms_title.name}{else}{$cms_title.category_name}{/if}</a></h4>
...
{else}
	<!-- MODULE Block footer -->
	<div class="span3" id="block_various_links_footer">
		<h4 class="block-title">{l s='Information' mod='blockcms'}</h4>
		<ul class="nav nav-list">
			{if !$PS_CATALOG_MODE}
			<li class="first_item"><a href="{$link->getPageLink('prices-drop')}" title="{l s='Specials' mod='blockcms'}">{l s='Specials' mod='blockcms'}</a></li>{/if}
