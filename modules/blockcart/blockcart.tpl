...
<!-- MODULE Block cart -->
<div id="cart_block" class="well-white exclusive">
...
{/foreach}
		...
		<div class="btn-group">
			{if $order_process == 'order'}
				<a href="{$link->getPageLink("$order_process", true)}" class="btn" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">{l s='Cart' mod='blockcart'}</a>
			{/if}
				<a href="{$link->getPageLink("$order_process", true)}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart'}" rel="nofollow" class="btn btn-primary"><span></span>{l s='Check out' mod='blockcart'}</a>
		</div>
	</div>
	</div>
</div>
