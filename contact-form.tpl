...
{else}
	...
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
		<fieldset>
			<legend>{l s='send a message'}</legend>
			<div class="row-fluid">
			<div class="span6">
			<p class="select">
			...
			<div class="span6">
				<p class="text">
						...
						{if isset($customerThread.email)}
							<input type="text" id="email" class="span12"  name="from" value="{$customerThread.email|escape:'htmlall':'UTF-8'}" readonly="readonly" />
						{else}
							<input type="text" id="email" class="span12"  name="from" value="{$email|escape:'htmlall':'UTF-8'}" />
						{/if}
				</p>
				...
				<p class="textarea">
					<label for="message">{l s='Message'}</label>
					 <textarea id="message" name="message" class="span12" rows="15" cols="10">{if isset($message)}{$message|escape:'htmlall':'UTF-8'|stripslashes}{/if}</textarea>
				</p>
				<p class="submit">
					<input type="submit" name="submitMessage" id="submitMessage" value="{l s='Send'}" class="btn btn-primary pull-right" onclick="$(this).hide();" />
				</p>
			</div>
		</div>
	</fieldset>
