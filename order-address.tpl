...
{/if}
<form action="{$link->getPageLink($back_order_page, true)}" method="post" class="">
	{else}
	...
	<div id="opc_account" class="opc-main-block">
		...
		<div class="addresses clearfix">
			<p class="address_delivery select ">
              ...
              <label class="control-label" for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Choose a billing address:'}{else}{l s='Choose a delivery address:'}{/if}</label>
              <div class="controls">
			...
			<p class="address_add submit">
				<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="btn">{l s='Add a new address'}</a>
			</p>
			{if !$opc}
		...
		<p class="cart_navigation submit">
			...
			<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}" title="{l s='Previous'}" class="btn pull-left">&laquo; {l s='Previous'}</a>
			<input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="exclusive btn btn-primary pull-right" />
		</p>
	</form>
	{else}
</div>
